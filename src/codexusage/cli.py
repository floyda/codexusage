...
def _merge_cfg(cfg: dict, args) -> dict:
    ...
    if getattr(args, "sessions_dir", None):
        ...
        if len(merged["projects"]) == 1 and merged["projects"][0]["name"] == "default":
            merged["projects"] = [
                {**merged["projects"][0], "sessions_dir": args.sessions_dir}
            ]
...
def _print_model_table(events: list[dict], pricing: dict, cpd: float, show_credits: bool) -> tuple[float, float, float]:
...
def cmd_config_init(args):
    from .config import _config_path, _default_sessions_dir, DEFAULTS
    path = _config_path()
    ...
    print("Projects:   codexusage config project add --name <name> --home <dir> --auth-type oauth|api_token")
...
def main():
    ...
    cadd_dir.add_argument("--home",         dest="home",         help="Codex home directory (sessions/ appended automatically)")
    cadd_dir.add_argument("--sessions-dir", dest="sessions_dir", help="Path directly to the sessions directory")
    cadd.add_argument("--auth-type", required=True, choices=["oauth", "api_token"],
                      dest="auth_type", help="oauth (counts toward credit pool) or api_token (USD billing)")
